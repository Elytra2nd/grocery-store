// resources/js/Pages/Admin/Orders/Processing.tsx
...
export default function ProcessingOrders({ orders, statistics, filters }: Props): JSX.Element {
    ...
    const handleSearch = (e: FormEvent<HTMLFormElement>): void => {
        ...
        router.get('/admin/orders/processing', searchParams as Record<string, any>, {
            preserveState: true,
            replace: true,
        });
    };

    const handleQuickAction = (orderId: number, action: 'ship' | 'complete'): void => {
        const order = safeOrders.find(o => o.id === orderId);
        if (!order) return;

        const confirmMessage = action === 'ship'
            ? `Tandai pesanan #${order.order_number} sebagai dikirim?`
            : `Tandai pesanan #${order.order_number} sebagai selesai?`;

        if (confirm(confirmMessage)) {
            router.patch(`/admin/orders/${orderId}/status`, {
                status: action === 'ship' ? 'shipped' : 'delivered',
                notes: action === 'ship' ? 'Pesanan sedang dikirim' : 'Pesanan selesai'
            });
        }
    };
                    ...
                    <div className="md:flex md:items-center md:justify-between mb-6">
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center">
                                <CogIcon className="h-8 w-8 text-blue-500 mr-3 animate-spin" />
                                <div>
                                    <h2 className="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl">
                                        Pesanan Sedang Diproses
                                    </h2>
                                    <p className="text-sm text-gray-600">
                                        Kelola pesanan yang sedang dalam proses
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        >
                            ...
                            <option value="ship_all">Kirim Semua</option>
...
// ProcessingOrdersTable component
interface ProcessingOrdersTableProps {
    orders: Order[];
    selectedOrders: number[];
    onToggleSelection: (id: number) => void;
    onToggleSelectAll: () => void;
    onQuickAction: (orderId: number, action: 'ship' | 'complete') => void;
...
function ProcessingOrdersTable({
    ...
    onQuickAction,
    formatCurrency
}: ProcessingOrdersTableProps): JSX.Element {
    // Helper function untuk menghitung progress
    const getProgress = (order: Order): { percentage: number; status: string } => {
        // Simulasi progress berdasarkan waktu sejak order dibuat
        const createdAt = new Date(order.created_at);
        const now = new Date();
        const hoursSinceCreated = (now.getTime() - createdAt.getTime()) / (1000 * 60 * 60);

        if (hoursSinceCreated < 1) {
            return { percentage: 25, status: 'Mempersiapkan pesanan' };
        } else if (hoursSinceCreated < 2) {
            return { percentage: 50, status: 'Mengemas produk' };
        } else if (hoursSinceCreated < 4) {
            return { percentage: 75, status: 'Siap untuk dikirim' };
        } else {
            return { percentage: 90, status: 'Menunggu pengiriman' };
        }
    };
        ...
        return (
            <div className="bg-white shadow rounded-lg">
                <div className="px-4 py-12 text-center">
                    ...
                    <h3 className="mt-2 text-sm font-medium text-gray-900">Tidak ada pesanan sedang diproses</h3>
                    <p className="mt-1 text-sm text-gray-500">
                        Semua pesanan sudah dikirim atau belum ada yang diproses.
                    </p>
                </div>
            </div>
        );
                                ...
                                <tr key={order.id} className="hover:bg-blue-50 transition-colors duration-150">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex space-x-2">
                                            <button
                                                onClick={() => onQuickAction(order.id, 'ship')}
